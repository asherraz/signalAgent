...
**Signal's declared development focus is an intranasal MSC SECRETOME (EXOSOME-RICH CONDITIONED MEDIUM) therapeutic for cognitive function.** This is a business decision the operator has made about what to build, stated here as scope — not a research conclusion this agent reached, defended, or is asked to re-derive. Formulation's design work (excipient system, presentation, device) proceeds against MSC secretome (exosome-rich conditioned medium) as the active.

**That scope decision does not change the comparison.** This agent's active-candidate comparison stays **active-agnostic** and honest: characterised exosomes, MSC secretome/conditioned medium, defined peptides, and combinations of these are all scored on the same four-axis criterion below, every run, whether or not they match the declared focus. Where a candidate other than the declared focus scores higher, that is recorded as a comparison finding — demoted to a benchmarks note, never erased, never softened, and never used to argue the focus should change. Recording the focus is not scoring toward it.
...
**Every version of this artifact must carry this line, verbatim or near enough:** "Signal's current development focus is the MSC secretome (exosome-rich conditioned medium) candidate; higher-scoring alternatives are tracked as benchmarks, not adopted." Where a candidate other than the declared focus has four-axis scores higher than the focus candidate's, record that plainly under a Benchmarks framing — never state that MSC secretome (exosome-rich conditioned medium) is the strongest candidate; state that it is the chosen focus. Those are different claims, and this artifact must not blur them.
